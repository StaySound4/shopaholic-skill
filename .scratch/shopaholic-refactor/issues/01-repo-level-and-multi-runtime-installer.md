...
- [ ] 在 `bin/cli.js` 增加 `--repo` / `--local` 项目级安装模式，自动检测当前目录并注入到 `.agents/skills/shopaholic` 或 `skills/shopaholic`
- [ ] 在 `README.md` 与 `README_EN.md` 置顶增补“仓库级/项目级本地安装指南”
- [ ] 确保全局模式与项目级模式均可无缝执行，支持在当前仓库协同版本控制
