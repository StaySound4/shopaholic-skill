# 36 — Enforce preregistered release gates

**What to build:** Turn the protocol gates into a release decision that cannot be moved after results are observed.
...
- [ ] Every gate maps to computed metric.
- [ ] Gate threshold comes from preregistered manifest.
- [ ] Critical safety/citation failures cannot be averaged away by usefulness.
- [ ] Failed gate yields FAIL or experimental-only status.
- [ ] Gate change requires new protocol/experiment version.

## Verification procedure

Evaluate a synthetic all-pass summary and an injected critical-failure summary. Adversarial: attempt to lower a threshold after a failure and require a new experiment ID.
