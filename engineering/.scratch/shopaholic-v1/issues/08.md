...
**What to build:** Create the product-identity hierarchy and ensure claim scope follows the narrowest evidence-supported identity.
...
- [ ] Region variants remain distinct when decision-relevant properties differ.
- [ ] Revision and batch can be represented independently.
- [ ] Unknown lower-level identity is not guessed.

## Verification procedure

Pass: same main-unit GTIN in different bundles resolves one canonical product; CN/US variants remain distinct. Adversarial: a revision with changed interface cannot inherit old-revision feature claims.
