...
**What to build:** Execute preregistered baseline, uploaded-current, target, required ablations, and controls on the fixed case set with preserved raw outputs and repeated runs.
...
- [ ] Manifest is preregistered before target runs.
- [ ] Every planned run has immutable artifact/status.
- [ ] Controls are evaluated before target interpretation.
- [ ] No manual output repair.
- [ ] Blocked/invalid counts are reported separately.
