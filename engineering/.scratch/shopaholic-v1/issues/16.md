...
**What to build:** After initial research, ask only questions whose answers would materially change the survivor set or ranking; otherwise skip Round 2.
...
- [ ] No generic mandatory used/report-format questionnaire.
- [ ] Questions already answered are suppressed.
- [ ] No material decision boundary -> no Round 2.

## Verification procedure

Pass: search finds used flagship at same price and asks used acceptance if category-eligible. Adversarial: if no used option or user already says new-only, do not ask.
