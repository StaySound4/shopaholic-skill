...
**What to build:** Make the exact uploaded Shopaholic behavior a hash-addressed baseline condition and establish the smallest run artifact contract needed to execute and preserve one real case.
...
- [ ] Baseline content is hash-addressed and cannot be silently edited in-place.
- [ ] One controlled case can be run in a fresh context and its verbatim output/metadata preserved.
- [ ] A run has an immutable run ID and explicit status taxonomy.
- [ ] No manually entered quality score is part of the run seam.
