...
2. **Phase 2（交付视角自适应选择）**：
   - 基于 Phase 1 萃取的用户画像，通过 `ask_user_question` 让用户从 3 种交付视角中选择：
     ...
     - ② **【按细分场景矩阵罗列】**：按不同使用场景（如：场景1通勤/场景2出游）分别列出从夯到拉的候选矩阵，杜绝单场景偏好误杀跨界品类；
     - ③ **【硬性特征黑名单剔除】**：对用户明确厌恶的特征（如“翻转屏/高碎镜风险/某材质”）做绝对排除后进行胜出款决战；
   - **视角适配映射**：用户选择视角后，直接进入【模式 B】并根据所选视角自适应调整第 4 节对比矩阵结构（① 统一排名表 / ② 多场景分栏表 / ③ 排除项前置说明 + 幸存者对比表）。
---
